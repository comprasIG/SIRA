...
const guardarOpcionesRfq = async (req, res) => {
    ...
    // --- ¡CORRECCIÓN! Obtenemos los archivos desde req.files ---
    ...
    try {
        ...
        // --- LÓGICA DE ARCHIVOS (CORREGIDA Y REUBICADA) ---
        // Borramos los adjuntos de cotización ANTERIORES para este RFQ para evitar duplicados.
        await client.query(`DELETE FROM rfq_proveedor_adjuntos WHERE requisicion_id = $1`, [requisicion_id]);
        
        if (files && files.length > 0) {
            // Creamos un mapa para buscar fácilmente la razón social de cada proveedor.
        ...
        // --- LÓGICA DE GUARDADO DE OPCIONES (SIN CAMBIOS, SOLO ORDENADA) ---
        const pendientesResult = await client.query(`SELECT id FROM requisiciones_detalle WHERE requisicion_id = $1 AND status_compra = 'PENDIENTE'`, [requisicion_id]);
        const idsPendientes = pendientesResult.rows.map(row => row.id);

        if (idsPendientes.length > 0) {
            await client.query(`DELETE FROM requisiciones_opciones WHERE requisicion_detalle_id = ANY($1::int[])`, [idsPendientes]);
        }
        ...
        for (const opt of opciones) {
            ...
            await client.query(
                `INSERT INTO requisiciones_opciones (requisicion_id, requisicion_detalle_id, proveedor_id, precio_unitario, cantidad_cotizada, moneda, seleccionado, es_precio_neto, es_importacion, es_entrega_inmediata, tiempo_entrega_valor, tiempo_entrega_unidad, subtotal, iva, ret_isr, total, config_calculo, es_total_forzado) 
                ...
                [
                    ...
                    opt.es_importacion, opt.es_entrega_inmediata, opt.tiempo_entrega_valor || null,
                    opt.tiempo_entrega_unidad || null, opt.seleccionado ? resumenProveedor?.subTotal : null,
                    opt.seleccionado ? resumenProveedor?.iva : null, opt.seleccionado ? resumenProveedor?.retIsr : null,
                    opt.seleccionado ? resumenProveedor?.total : null, opt.seleccionado ? resumenProveedor?.config : null, 
